...
if(!isset($_GET['dl_path']) || empty($_GET['dl_path'])
		|| strpos($_GET['dl_path'], '/') !== false   // Preventing dot dot slash attack
		|| strpos($_GET['dl_path'], '\\') !== false) // Preventing dot dot slash attack
{
    get403();
    die();
...
$target = "../view/{$_GET['dl_path']}";

if(file_exists($target))
{
    set_time_limit(0);
    $download = new ResumeDownload($target, 0); //If set delay to non-zero (even 1), speed will be reduced.
    $download->process();
}
else
    get404();
...
//From https://gist.github.com/kosinix with some changes
...
class ResumeDownload {
    ...
    function __construct($file, $delay = 0, $dlName = '') {
